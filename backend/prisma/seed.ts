...
async function main() {
  ...
  // ==================== USERS ====================
  // Passwords are stored as bcrypt hashes — plain-text values shown here
  // are only for developer reference (printed to console after seeding).
  const users = [
    { email: 'admin@cloudpos.com',      password: 'Admin@1234',      full_name: 'System Admin',    role: UserRole.ADMIN,         phone: '+1234567890' },
    { email: 'supervisor@cloudpos.com', password: 'Super@1234',      full_name: 'Shift Supervisor', role: UserRole.SUPERVISOR,    phone: '+1234567891' },
    { email: 'stock@cloudpos.com',      password: 'Stock@1234',      full_name: 'Stock Manager',   role: UserRole.STOCK_MANAGER, phone: '+1234567892' },
    { email: 'cashier@cloudpos.com',    password: 'Cashier@1234',    full_name: 'Main Cashier',    role: UserRole.CASHIER,       phone: '+1234567893' },
  ];
  ...
  for (const u of users) {
    ...
    await prisma.user.upsert({
      where: { email: u.email },
      update: { password: hashedPassword }, // re-hash on every seed run
      create: {
        email:     u.email,
  ...
  console.log('\n✅ Seed complete!');
  console.log('\n📋 Login Credentials (passwords are bcrypt-hashed in DB):');
  console.log('   Admin:      admin@cloudpos.com      / Admin@1234');
  console.log('   Supervisor: supervisor@cloudpos.com / Super@1234');
  console.log('   Stock Mgr:  stock@cloudpos.com      / Stock@1234');
  console.log('   Cashier:    cashier@cloudpos.com    / Cashier@1234');
}
